...
class IndividualCatalogCreatorFactory(BaseCreatorFactory):
    ...
    def __init__(self, data_root, config, deployment_dicts = {}):
        self.config = config
    ...
    def get_deployments_to_update_generator(self):
        ...
        for deployment in self.deployment_dicts:
            ...
            if not dataset_chunk_path:
                # check if there is nc file under the folder, if doesn't skip
    ...
    def generate(self):
        ...
        individual_xml_creators = []
        logger.info("Selecting deployment for update")
        for deployment_dict in self.get_deployments_to_update_generator():
            individual_xml_creators.append(IndividualCatalogGenerator(deployment_dict, self.config))
        logger.info("{} deployments was selected to update".format(len(individual_xml_creators)))
        return individual_xml_creators

#
# class CatalogCreatorFactory(BaseCreatorFactory):
#     # todo: this section should be replaced by the datasets_catalog_crafter
#     def __init__(self, data_root, catalog_root, deployment_dicts={}):
#         """
